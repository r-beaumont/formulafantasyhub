...
export default function SubscribePage() {
  const card = { background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '14px' }
      ...
      <div className="mob-pad-sub" style={{ position: 'relative', zIndex: 1, maxWidth: '1000px', margin: '0 auto', padding: '60px 32px' }}>
        ...
        <div className="mob-1col" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '40px' }}>
          ...
          <div style={{ ...card, padding: '32px' }}>
            <div style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '2px', color: 'var(--muted)', textTransform: 'uppercase', marginBottom: '8px' }}>Free</div>
            <div style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '56px', lineHeight: 1 }}>€0</div>
            <div style={{ color: 'var(--muted)', fontSize: '13px', marginBottom: '32px' }}>per month, forever</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '32px' }}>
              {freeItems.map((f) => (
                <div key={f.label} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '13px', color: f.free ? '#00D47E' : 'var(--muted2)', flexShrink: 0 }}>
                    {f.free ? '✓' : '🔒'}
                  </span>
                  <span style={{ fontSize: '13px', color: f.free ? 'var(--text)' : 'var(--muted2)' }}>{f.label}</span>
                </div>
              ))}
            </div>
            <Link href="/articles" style={{ display: 'block', textAlign: 'center', padding: '12px', border: '1px solid var(--border)', borderRadius: '8px', color: 'var(--text)', textDecoration: 'none', fontSize: '13px', fontWeight: 600 }}>
              Read free articles
            </Link>
          </div>
          ...
          <div style={{ background: '#E8002D', borderRadius: '14px', padding: '32px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '56px', lineHeight: 1, color: 'var(--text)' }}>€5</div>
            <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: '13px', marginBottom: '32px' }}>per month · cancel anytime</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '32px' }}>
              {freeItems.map((f) => (
                <div key={f.label} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '13px', color: 'var(--text)', flexShrink: 0 }}>✓</span>
                  <span style={{ fontSize: '13px', color: 'var(--text)', fontWeight: 500 }}>{f.label}</span>
                </div>
              ))}
            </div>
            <button style={{ width: '100%', background: 'var(--text)', color: '#E8002D', border: 'none', borderRadius: '8px', padding: '14px', fontSize: '14px', fontWeight: 700, cursor: 'pointer', letterSpacing: '0.3px' }}>
        ...
        <div style={{ ...card, padding: '32px', marginBottom: '60px' }}>
          <div style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '2px', color: 'var(--muted)', textTransform: 'uppercase', marginBottom: '24px' }}>What's included in Premium</div>
          {groups.map((group, gi) => (
            <div key={group.label} style={{ borderTop: gi === 0 ? 'none' : '1px solid var(--border)', paddingTop: gi === 0 ? '0' : '24px', marginTop: gi === 0 ? '0' : '24px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {group.items.map((item) => (
                  <div key={item} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                    ...
                    <span style={{ fontSize: '13px', color: group.coming ? 'var(--muted)' : 'var(--text)', lineHeight: 1.6 }}>
                      {item}
                      {group.coming && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
            {faqs.map((f) => (
              <div key={f.q} style={{ ...card, padding: '20px 24px' }}>
                ...
                <div style={{ color: 'var(--muted)', fontSize: '14px', lineHeight: 1.6 }}>{f.a}</div>
              </div>
            ))}
          </div>
        </div>
