...
export default function StandingsClient() {
  ...
  // Compute constructor wins/podiums/poles from driver data
  const constructorStats = drivers.reduce((acc, d) => {
    const key = d.team
    if (!acc[key]) acc[key] = { wins: 0, podiums: 0, poles: 0 }
    acc[key].wins += d.wins
    acc[key].podiums += d.podiums ?? 0
    acc[key].poles += d.poles ?? 0
    return acc
  }, {} as Record<string, { wins: number, podiums: number, poles: number }>)
  ...
  const constructors = [...CONSTRUCTOR_STANDINGS]
    ...
    .sort((a, b) => {
      ...
      if (conSortMode === 'poles')   return dir * ((constructorStats[b.name]?.poles ?? 0) - (constructorStats[a.name]?.poles ?? 0))
  ...
  return (
    <div className="mob-pad-page" style={{ position: 'relative', zIndex: 1, maxWidth: '1400px', margin: '0 auto', padding: '28px 32px 60px' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <div style={{ color: 'var(--muted)', fontSize: '13px', marginTop: '6px' }}>Live data — updates after each race · After R7 Barcelona-Catalunya</div>
      </div>
      ...
      {(() => {
        const raceWinners = completedCalRounds.map(r => RACE_WEEKENDS[r.round]?.race?.[0]?.name).filter(Boolean) as string[]
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '14px', overflow: 'hidden', marginBottom: '24px' }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          ...
          <div style={{ padding: '8px 24px 20px', display: 'flex', flexDirection: 'column', gap: '2px', minWidth: '720px' }}>
            {sorted.map((d, i) => {
              const rank = i + 1
              const barPct = maxDriverPts > 0 ? (d.points / maxDriverPts) * 100 : 0
              return (
                <div key={d.id} style={{ display: 'grid', gridTemplateColumns: '28px 220px 120px 1fr 56px 52px 52px 52px', alignItems: 'center', gap: '12px', padding: '7px 0', borderBottom: '1px solid rgba(255,255,255,0.03)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          ...
          <div style={{ padding: '8px 24px 20px', display: 'flex', flexDirection: 'column', gap: '12px', minWidth: '720px' }}>
            {constructors.map((c, i) => {
              ...
              return (
                <div key={c.name} style={{ display: 'grid', gridTemplateColumns: '28px 220px 120px 1fr 56px 52px 52px 52px', alignItems: 'center', gap: '12px' }}>
